...
select is(public.get_workloop_access()->>'state','trial','first public use starts trial');
select is(public.get_workloop_access()->>'has_access','true','trial allows use');
select is((public.get_workloop_access()->>'trial_ends_at')::timestamptz,now()+interval '720 hours','trial is exactly30elapsed days');
select is((public.get_workloop_access()->>'trial_ends_at')::timestamptz,now()+interval '720 hours','repeated checks cannot restart trial');
...
-- completes. The exception must roll the newly started trial back as well.
...
reset role;
select is((select count(*) from app_private.account_access where user_id='a2600000-0000-4000-8000-000000000002'),0::bigint,'failed in-transaction recheck does not start a trial');
