-- Make onboarding a single authenticated, atomic operation and preserve
-- deletion requests when their workspace is removed so completion can retry.
...
alter table public.account_deletion_requests
  drop constraint if exists account_deletion_requests_workspace_id_fkey;

alter table public.account_deletion_requests
  add constraint account_deletion_requests_workspace_id_fkey
