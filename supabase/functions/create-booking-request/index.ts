...
Deno.serve(async (req: Request) => {
  ...
  // This endpoint uses the service role, so explicitly reject stale public
  // profiles whose workspace no longer has an owner/member. The database
  // trigger added with this boundary provides the final insert-time defence.
  ...
  if (!member) return response(404, { error: "Profile not found" });
