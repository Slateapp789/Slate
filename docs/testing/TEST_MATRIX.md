...
| Main navigation and deep links | High | Partial | Covered for global implicit/explicit vertical scroll targeting, retained-tab independence, uniform programmatic shell transitions, reduced motion, native and fallback back-swipe completion, direct-route back actions, protected draft gestures, keyboard dismissal, and shared nav | Harness only for signed-out Auth navigation | Not applicable | Real notification, password-reset, public-profile, shell deep links, and physical gesture feel still need iOS/Android device checks. |
| Shared design system | High | Covered for Light/Dark semantic contrast, warm-neutral Light layers, selective semantic accent borders, persisted System/Light/Dark state, native startup colour, pickers, and reusable logic | Covered across primitive, state, trust, accessibility, 288 cross-appearance launch-surface combinations, and reviewed Light/Dark goldens including Dashboard Light | None | Not applicable | Automated contrast and layout contracts pass; physical-device accessibility review remains open. |
...
| Booking schedule and calendar | Critical | Covered for legacy recurrence compatibility, validation, conflict mapping, price, workflow payloads, time/date boundaries, and status filtering | Partial: selectors, controls, forms, detail, empty/populated and responsive states; recurring-series creation is intentionally absent from V1 | None authenticated | Partial: workflow/function contracts and appointment tenant boundaries | Real create/edit/reschedule/complete/cancel, conflict races, and DST behaviour need isolated E2E. Existing recurring records remain readable, but no new series promise is exposed. |
| Public profile | Critical | Partial: repository/model and public-response handling | Partial: public-profile and booking-form UI audit plus golden surface | Staging harness compiles and covers signed-out profile load, honeypot, invalid service and duplicate-token behaviour; execution is configuration-gated | Static Edge boundary inspected | Deployed isolated execution, rate-limit/recovery cases and physical signed-out device access remain open. |
| Booking requests | Critical | Covered for required email validation, confirmation outcome mapping, stable retry/idempotency contract, lifecycle/error mapping, and backend security source contracts | Public receipt/privacy purpose, populated owner inbox/detail, read-only delivery address and legacy no-email conversion are covered | Staging harness compiles through signed-out submit, owner visibility and atomic booking/payment conversion; execution is configuration-gated | Local replay passes 26 confirmation-email pgTAP assertions for normalization, private outbox, atomic enqueue, retry/lease/backoff and contact preservation | Run the harness with a controlled inbox; prove exactly-one delivery, provider outage/scheduled recovery, auth headers, bounce/suppression, decline, preferences and throttling. |
...
| Settings and appearance | High | Covered for persisted System/Light/Dark and map preference state; account hardening remains partial | Covered for the dedicated appearance destination, core Settings ownership, trust, Light/Dark goldens, and responsive contracts | None authenticated | Indirect | Appearance persistence and rendering are automated; physical theme switching, password update, sign-out, and production recovery still need device checks. |
...
| Edge Functions | Critical | 38/38 Deno tests cover selected shared/booking/email/Places/Stripe behaviours; ten entry points are explicitly checked by local/hosted gates | Not applicable | None against isolated deployments | Static handler checks plus local outbox/grant assertions pass | Deployed public request, confirmation/drain, profile, Places, deletion, Stripe, rate-limit, timeout and idempotency paths need disposable staging journeys. |
| Supabase RLS and tenant isolation | Critical | Not applicable | Not applicable | Two-user SDK harness attempts victim reads/updates/deletes and attacker inserts across 13 practical disposable tables, including relationship children and device-token/calendar rows; execution is staging-credential-gated | Empty local replay passes 109 pgTAP assertions across schema/security, two-user, MFA/payment-retention and booking-email boundaries | Disposable hosted staging execution and live migration-history reconciliation remain open; local Postgres is not production parity. |
...
| Stripe payment collection | Critical, release-gated | Partial: payment/refund mapping, repository state and Edge helpers | Partial: setup, collection, links, receipts, processing and refund states | No complete provider/device journey | Static payment-ledger/RLS/function contracts; deployed live configuration recorded | Merchant onboarding, account webhook, payment link, receipt, refund, retry and bounded live-money evidence remain open; Tap to Pay also awaits Apple entitlement. Subscription entitlement remains out of scope. |
...
| iOS platform/build | Critical | Not applicable | Automated viewport coverage plus reviewed goldens | Signed-out simulator smoke currently fails on Auth toggle reachability | Not applicable | Distribution signing/export works locally, but build 3 is stale/not accepted; current physical workflow, VoiceOver, permissions, lifecycle, reinstall and upgrade remain release gates. |
| Web legal/support surface | Critical external | Not applicable | Public source artifacts contain canonical/support/Stripe/current-navigation content | Prior manual HTTP checks recorded HTTP 200 | Not applicable | Legal-controller details, support monitoring, consistent host routing and launch-market legal approval remain store gates. |
...
- Golden and integration:
  ...
  [`staging_core_workflow_test.dart`](../../integration_test/staging_core_workflow_test.dart),
  [`staging_tenant_isolation_test.dart`](../../integration_test/staging_tenant_isolation_test.dart),
  [`staging_public_booking_test.dart`](../../integration_test/staging_public_booking_test.dart)
- Database:
