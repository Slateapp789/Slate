...
| Booking requests | Critical | Covered for validation, confirmation mapping, stable retry/idempotency contract, lifecycle/error mapping, and backend security source contracts | Populated inbox and detail plus public request controls have reviewed golden coverage | Staging harness compiles through signed-out submit, owner visibility and atomic booking/payment conversion; execution is configuration-gated | Partial: booking-request table and workflow contracts; dynamic pgTAP pending | Run the harness on disposable staging, then add concurrent-submit, decline, preference and throttling evidence. |
...
| Edge Functions | Critical | Partial: 25 Deno tests cover selected shared/booking/Places/Stripe behaviours | Not applicable | None against isolated deployments | Static type/grant/function inspection; local database tests pending | Deployed public request, profile, Places, deletion, Stripe, rate-limit, timeout and idempotency paths need disposable staging journeys. |
| Supabase RLS and tenant isolation | Critical | Not applicable | Not applicable | Two-user SDK harness attempts victim reads/updates/deletes and attacker inserts across 13 practical disposable tables, including relationship children and device-token/calendar rows; execution is staging-credential-gated | Harness only: 51 schema/security, 16 two-user boundary and 15 privileged-MFA/payment-retention assertions are authored | Docker/local stack and disposable staging execution remain open; CI configuration is not hosted passing evidence. |
