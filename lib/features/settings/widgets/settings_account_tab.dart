...
class _SettingsAccountTabState extends ConsumerState<SettingsAccountTab> {
  ...
  Future<void> _changePassword() async {
    ...
    try {
      ...
      if (_isCurrentPage) _snack('Password updated', AppColors.green);
    } catch (_) {
        ...
        builder: (ctx, updateSheet) {
          ...
          return _AccountSheet(
            ...
            children: [
              ...
              saveBtn(label: 'Save name', onTap: save, loading: saving),
  ...
  Future<void> _exportData() async {
    ...
    try {
      ...
      if (_isCurrentPage && savedPath != null) {
    ...
    await _showAccountSheet(
      (ctx) => StatefulBuilder(
        builder: (ctx, updateSheet) => _AccountSheet(
          ...
          children: [
            const Text(
              'Export any records you need before continuing. Deleting your Workloop account does not cancel an Apple or Google subscription; cancel it in your store subscription settings first if you have one. We’ll email you when deletion is complete. Some records may need to be kept for legal or security reasons.',
              style: TextStyle(color: AppColors.t2, height: 1.5),
            ),
            ...
            saveBtn(
              ...
              onTap: () async {
                ...
                try {
                  ...
                  accepted = true;
                  if (!_isCurrentAccount) return;
                  // Deletion is already accepted. Optional local cleanup cannot
                  // turn that successful request into a reported failure.
                  try {
                    await ref.read(onboardingProvider.notifier).clearDraft();
                  } catch (_) {
                    /* Best-effort cleanup after accepted deletion. */
                  }
                  await auth.signOutLocal(expectedUserId: _accountId);
                  if (!mounted ||
                      (auth.currentUserId != null &&
                          auth.currentUserId != _accountId)) {
                    return;
                  }
                  if (ctx.mounted && ModalRoute.of(ctx)?.isCurrent == true) {
                    Navigator.pop(ctx);
                  ...
                  if (ctx.mounted && _sheetIsCurrent(ctx)) {
                    updateSheet(
                      () => error = accepted
                          ? 'Your deletion request was accepted. Close and reopen Workloop to finish signing out.'
                          : 'Your deletion request could not be confirmed. Please try again or contact support.',
                    );
                  }
                  if (accepted && _isCurrentAccount) {
                    ref.invalidate(sessionIntegrityProvider);
                    ref.invalidate(workspaceProvider);
                  }
                } finally {
              ...
              child: Column(
                ...
                children: [
                  Text(
                    ...
                    style: const TextStyle(color: AppColors.t2, height: 1.5),
                  ),
                  const SizedBox(height: AppSpacing.md),
                  TextField(
...
class _AccountActionRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = destructive ? AppColors.error : AppColors.t1;
