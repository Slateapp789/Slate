...
void main() {
  testWidgets('long income history keeps the complete overview ahead of rows', (
    tester,
  ) async {
    ...
    final setupY = tester.getTopLeft(find.text('Get paid with Workloop')).dy;
    final historyY = tester.getTopLeft(find.text('Payments')).dy;
    expect(chartY, lessThan(targetY));
    expect(targetY, lessThan(setupY));
    expect(setupY, lessThan(historyY));
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        ...
        lessThan(tester.getTopLeft(find.text('Expenses')).dy),
      );
